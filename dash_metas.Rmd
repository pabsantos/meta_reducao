...
metas1 <- metas %>% 
  select(codigo, sigla_uf, nome, regiao, porte, media_mortes, var_perc) %>% 
  rename(meta_porcentagem = var_perc) %>% 
  mutate(meta_porcentagem = scales::percent(meta_porcentagem, accuracy = 1),
...
```{r table}
datatable(metas1, filter = "top", rownames = FALSE, height = NULL, 
          options = list(searchHighlight = TRUE, scrollY = 700, bPaginate = FALSE))
```
